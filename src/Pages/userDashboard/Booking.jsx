...
function Booking() {
  ...
  // ==================================================
  // CHECK WHETHER USER ALREADY BOOKED THIS CLASS
  // ==================================================
    ...
    const checkExistingBooking = async () => {
      try {
        ...
        const response = await api.get(
          `/bookings/user/${loggedInUser._id}`
        );
        ...
        const existingBooking = bookings.some(
          (booking) => {
            ...
            return (
              String(bookedClassId) ===
              String(id)
            );
          }
        );

        setAlreadyBooked(existingBooking);

        // Clear selected slot if already booked
        if (existingBooking) {
          setSelectedSlot("");
        }
      } catch (error) {
  ...
  const getClassDateTime = (slot) => {
    ...
    // Example:
    // 7:00 AM - 8:00 AM
    // Only starting time is required
  ...
  const isBookingAllowed = () => {
    ...
    // Class must be at least 24 hours
    // from current time
    ...
    return (
      <div className="min-h-[70vh] flex items-center justify-center px-4 py-10">
        <div className="bg-white rounded-2xl shadow-md border border-gray-100 px-8 py-10 text-center">
          ...
          <p className="text-gray-500 mt-2">
            Please wait while we check your booking.
          </p>
        </div>
      </div>
    );
              ...
              <div className="mb-8 bg-yellow-50 border border-yellow-200 rounded-xl p-5">
                <div className="flex items-start gap-4">
                  ...
                  <div>
                    ...
                    <p className="mt-1 text-yellow-600 leading-6">
                      You have already booked this
                      class. You cannot book the
                      same class again.
                    </p>
                  </div>
                </div>
